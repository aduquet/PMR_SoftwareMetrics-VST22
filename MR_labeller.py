...
def labeller(df_f, df_mr):
    ...
    for index, row in df_f.iterrows():
        method_id = index.split('.')[0]
        df_f.at[index, 'method_id'] = method_id

    #for i in columNameMR:
    for index, row in df_f.iterrows():
        name = row.at['method_id']
        match = list(df_mr['Method_Name'].str.match(name)).index(True)
        for i in columNameMR:
            df_f.at[index, i] = df_mr.at[match, i]
